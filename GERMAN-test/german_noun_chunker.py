...
def run_chunker(list_name):
    ...
    list_name_without_tag = list_name.rsplit('.', 1)[0]
    # for every line that is read, use noun chunker on it and output .tchunk file
    # os.chdir("/"+list_name_without_tag)
    with open(list_name_without_tag+".txt", "r", encoding='utf-8') as input_file, open(list_name_without_tag+".tchunk", "w", encoding='utf-8') as output_file:
